...
export default class Root{
    static init(port=this.#port, callback=()=>{}){
        ...
        if(this.#notFoundPage === "")this.makeNotfoundPage();
        this.loadAccesses();
    ...
    // NOTFOUNDPAGE
    static notFoundPageDefault = VieuxjsDirectoriesFile.notFoundPage;
    static #notFoundPage = "";
    static get notFoundPage(){
        return  this.#notFoundPage;
    }
    static #notFound = (req, res) => {res.status(404).send(this.#notFoundPage);};
    static notFound(fnc){
        if(typeof fnc !== "function")throw new Error("");
        this.#notFound = fnc;
    };
    static makeNotfoundPage(){
        this.#notFoundPage = (new Page(this.notFoundPageDefault)).result;
    }
