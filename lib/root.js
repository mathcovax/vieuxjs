...
export default class Root{
    ...
    static init(port=this.#port, callback=()=>{}){
        ...
        if(this.#assetsDir !== false)this.#app.use(this.#assetsUrl, express.static(this.#assetsDir));
    ...
    static makeImportBody(){
        ...
        while(importBody.document.body.querySelector("div[data-c]")){
            for(const component of importBody.document.body.querySelectorAll("div[data-c]")){
                if(component.dataset.c.endsWith(".*")){
                    ...
                    for(const key in Root.components){
                        if(key.startsWith(component.dataset.c.replace(".*", ""))){
                            let div = importBody.document.body.createElement("div");
                            div.dataset.c = key;
                    ...
                    const div = importBody.document.body.createElement("div")
        ...
        for(const script of importBody.document.querySelectorAll("script")){
            const style = importBody.document.createElement("style");
            style.type = "script";
        ...
        try{
            for(const style of importBody.document.body.querySelectorAll("style[type='scss']")){
