...
export default class Root{
    ...
    static addComponent(component){
        if(this.#components[component.name])Err("componentAlreadyExist", component.name);
        this.#components[component.name] = component;
        return component;
    }
    static getComponent(componentName){
        if(!this.#components[componentName])Err("componentNotExist", componentName);
    ...
    static addPage(path, page){
        ...
        if(this.#pages[path])Err("pageAlreadyExist");
        this.#pages[path] = {
            access: ()=>{return true},
            page: page,
            e: async (req, res) => {
                switch (await this.#pages[path].access(req, res)) {
                    case true:
                        res.set("params", JSON.stringify(req.params));
    ...
    static getPage(path){
        path = this.pathCorrector(path);
        if(!this.#pages[path])Err("pageNotExist");
    ...
    static addAccess(path, type="all", fnc){
        path = this.pathCorrector(path);
        if(type !== "method" && type !== "page" && type !== "all")Err("badTypeAccess");
        if(
            (type === "all" && (this.#accesses.page[path] || this.#accesses.method[path])) ||
            (type === "page" && this.#accesses.page[path]) ||
            (type === "method" && this.#accesses.method[path])
        )Err("accessAlreadyExist");
    ...
    static getAccess(path, type){
        path = this.pathCorrector(path);
        if(type !== "method" && type !== "page")Err("badTypeAccess");
        if(!this.#accesses[type][path])Err("accessNotExist");
    ...
    static addMethod(path, fnc){
        path = this.pathCorrector(path);
        if(this.#methods[path])Err("methodAlreadyExist");
        this.#methods[path] = {
            handlers: [],
            access: ()=>{return true},
            fnc: fnc,
    ...
    static getMethod(path){
        path = this.pathCorrector(path);
        if(!this.#methods[path])Err("methodNotExist");
    ...
    static addSocket(socket){
        if(this.#sockets[socket.name])Err("socketAlreadyExist");
        this.#sockets[socket.name] = socket;
        return socket;
    }
    static getSocket(name){
        if(!this.#sockets[name])Err("socketNotExist");
